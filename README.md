...
如果你想先快速把握本项目的流程观、工具链和方法论演进，先读这份盘点：

- [流程工具方法经验全景盘点.md](./流程工具方法经验全景盘点.md) - 本项目的全景收口说明，适合当作 README 之后的第二入口
- [流程工具方法经验全景盘点短稿.md](./流程工具方法经验全景盘点短稿.md) - 面向上游或外部对话的精简收口稿
...
- **建设性意见已进入核心能力**：建设性意见不再是附加话术，而是导师层的默认标准能力；一旦任务进入导师修改建议阶段，就按固定链路生成方法学修复、分层建议、语言校准与必要的角色投影
...
1.3.1 如果进入导师层，默认启动建设性意见标准链路：`方法学修复路径表 → 导师分层修改建议（L1-L4）→ 流调/生统语言校准 → 角色/专业投影`
1.3.2 只有当建议进入 `L4` 或用户明确要求最新进展 / 创新定位时，才额外触发前沿检索 gate
1.4  冻结核心结果真源与依赖关系
1.5  判断是否跨代理协作场景
1.6  判断是否具备复算条件
...
1. **默认能力**
   建设性意见已经是本 skill 的默认能力之一，只要任务进入导师层，就必须按标准链路启用，不是可选插件。
2. **默认热路径**
   为了降低入口认知负担，建设性意见相关模块不在任务一开始就预加载，而是在进入导师层时显式触发。
